...
<div style="display: flex; align-items: center; margin-bottom: 10px;">
    ...
    <div style="background-color: #00ff78; padding: 10px; border-radius: 10px; max-width: 75%; word-wrap: break-word; overflow-wrap: break-word;">
        {msg}
    </div>
</div>
'''
...
def prepare_and_split_docs(file_path):
    split_docs = []
...
def get_conversation_chain(retriever):
    ...
    contextualize_q_system_prompt = (
        ...
        "Be concise but provide complete answers when necessary."
...
uploaded_files = st.sidebar.file_uploader("Upload CSV Documents", type=["csv"])

if uploaded_files:
    if st.sidebar.button("Process csvs"):
